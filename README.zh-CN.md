...
- 原生物品和词缀选择器支持 ASCII 空格分词搜索：输入两个或更多词时，候选须包含全部词，因此 `金 戒指` 可匹配 `金光戒指`，且命中词会分别高亮。单词搜索和手动输入的 `~` 查询保持官方 Trade 行为。
- 自动识别当前类别或物品名称。
- 按爬取到的 PoE2DB 数据过滤不属于当前类别的词缀建议。
- 支持识别失败时手动指定类别。
- 在支持的词缀筛选项左侧增加蓝色 Tier 选择器。已选物品类别时只显示该类别的 Tier；未选类别时会标注兼容的类别。
- 在市集结果行中增加用于 PoB Create Custom 的复制按钮。
- 为固定价的崇高石、混沌石、神圣石报价增加 `E`、`C`、`D` 换算按钮；汇率按当前市集赛季从 Poe2Scout 实时获取，面板中的刷新按钮可强制更新报价。

## 词缀等级选择

在扩展面板中开启“词缀等级选择”。支持的词缀筛选项会在文案左侧显示蓝色 `T` 按钮，点击后可选择 Tier。

- **最低**是默认模式，只写入 `MIN`，保留用户已填写的 `MAX`。每个 Tier 的 `MIN` 取下一低档 Tier 的最大平均值加 `0.1`，使结果不会再落入下一低档；最低 Tier 会留空 `MIN`。
- **精确**会同时写入所选 Tier 的平均 `MIN` 和 `MAX` 范围。

Tier 映射由已验证的 PoE2DB 词缀范围生成，并按官方 Trade 稳定词缀 ID 匹配；无法唯一映射的词缀不会显示该选择器。
