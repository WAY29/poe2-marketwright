...
在“其他”筛选器中，`腐化 +1 最终宝石等级` 输入框可通过间接条件搜索因腐化获得一级的主动技能宝石。输入 `2` 至 `21` 的最终等级；例如输入 `20` 会将宝石等级固定为 `20-20`，把角色需求等级最大值设为 19 级宝石的需求等级，并将“已污染”设为“是”。

该预设不会修改当前已选的宝石名称或物品分类。使用前请先选择主动技能宝石，之后仍可直接使用原生筛选器继续细化条件。
...
- 物品显示名：简体中文 `3834/3834`（100%），繁体中文 `3834/3834`（100%）。
- Trade 词缀模板：简体中文 `8087/8186`（98.79%），繁体中文 `8093/8186`（98.86%）。
